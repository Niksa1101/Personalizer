-- Personalizer — import_batches.exists_list + import_commit() corrections
-- Spec: docs/DB.md §5.2; Phase 6 review fix plan §1.
--
-- Forward-only corrective migration: adds exists_list column (missed when
-- 20260723180000 was edited in place) and replaces import_commit with:
--   • persists exists_list on commit
--   • gates exists_list to linked rows only (not same-campaign duplicates)
--   • SECURITY INVOKER (service_role caller already holds table privileges)
--
...
-- calling this function — mirror resolveUniqueBatchSlug() in lib/imports.ts.

ALTER TABLE public.import_batches
  ADD COLUMN IF NOT EXISTS exists_list jsonb NOT NULL DEFAULT '[]'::jsonb;
