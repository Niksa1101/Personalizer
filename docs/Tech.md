...
When intro and recording lengths differ in `fullscreen_intro` mode, no reconciliation is needed: `lib/video/merge-plan.ts` returns `stretchGraph: 1`, `holdMs: 0`, `applySetpts: false`, and `targetMs = dIntroMs + dRecMs`. FFmpeg `concat` plays intro then recording at native speed; the output duration is simply the sum of the two inputs. Verified in `merge-plan.test.ts` and `verify:merge`.
...
**That rule is asserted, not trusted.** The preflight step base64-decodes the JWT payload of `SUPABASE_ANON_KEY` and fails the run if the role claim is the privileged one. This is the only place the invariant can actually be broken — both keys sit on the same dashboard page, and pasting the wrong one is silent: the insert would simply succeed. `verify:keepalive` O11 asserts the guard is still in the workflow, and N10 applies the same check to the key in `.env.local`. Keys that are not JWTs (`sb_publishable_*`) carry no role claim and pass through unexamined.
...
The UI scripts need both the dev server **and** Chromium (`npm run setup:browser`); without a server every leg reports **skipped**, never passed. Two of `verify:leads-ui`'s eleven legs additionally need **Google Chrome** installed (not bundled Chromium) to decode H.264 — see §16.2. Failure screenshots are written to `.verify-ui/` (gitignored) — a red leg with no picture costs a whole re-run.
...
**Scope a `catch` that skips to the one call that can legitimately be absent.** A `try` wrapping the whole leg with `catch → skip` turns every later defect into a silent pass: the playback legs originally reported a broken `/leads` page as *"Google Chrome not available"* and exited 0, because navigation and assertions sat inside the same `try` as the browser launch. Only the launch may skip; everything after it fails. Prove it with a mutation that throws *after* the optional dependency is satisfied — a bogus channel and an unreachable URL must produce different verdicts.
...
Note that `getRedis()` uses `lazyConnect`, so `status` sits at `"wait"` until something issues a command: any Redis-liveness check in a script that does not enqueue in-process must **connect** rather than read `status`, or it will conclude Redis is down while it is running.
...
Run `npm run verify:record` after installing Chromium to exercise the hermetic fixture leg. The network-dependent real-site leg is gated behind `RECORD_REAL=1`. The same Chromium install is what `npm run verify:page` and most of `npm run verify:leads-ui` drive.

**In-drawer video playback needs Google Chrome, not bundled Chromium.** The pipeline encodes H.264 + AAC; Playwright's open-source Chromium build cannot decode either. The two playback legs in `verify:leads-ui` launch `{ channel: "chrome" }` in a separate browser instance — the other nine legs stay on bundled Chromium so Phase 13 evidence still describes what ships. When Chrome is absent (CI, a fresh Windows box) both legs report **skipped** and the script exits 0; a pre-flight `canPlayType('video/mp4; codecs="avc1.42E01E"')` check keeps the legs honest if someone later switches the launch back to bundled Chromium.
...
| 11 | **A second worker against the same project** | **Closed in Phase 18** — `acquireWorkerSlot()` boot mutex + live liveness scan in `worker/index.ts`; hard-killed worker keys expire within 15 s. Bypass `PZ_ALLOW_MULTIPLE_WORKERS=1` for test harnesses only. |
...
2. **`fullscreen_intro` layout** — **closed as stale (Phase 18).** §9.2 documents the implemented behavior: concat at native speed, `targetMs = dIntroMs + dRecMs`.
...
13. **Heartbeat retention** — **closed in Phase 18.** Daily cleanup job runs `pruneHeartbeats()` (90-day retention, honors `cleanup.dry_run`).
14. **CI workflow** (typecheck/lint/test on every push) — **closed in Phase 18.** `.github/workflows/ci.yml` on `windows-latest`.
15. **The `schedule` trigger on supabase-keepalive** — still unobserved. `workflow_dispatch` was exercised on 2026-08-01 (run #1, green, row delta confirmed); the cron is a different code path in GitHub's scheduler and proves itself at 06:17 UTC the next day or not at all. **Open** until an `event: schedule` run appears in the Actions tab. This is risk 10 in concrete form: the failure mode is silence, so nothing will alert you — check with `npm run verify:keepalive -- --observe` in a couple of days and expect `count(*)` to have grown.
16. ~~**README / `verify:shell` demo campaign URL**~~ — **resolved.** Both referenced `/campaigns/00000000-…0001`, which `seed_demo_data()` never creates (it inserts with `gen_random_uuid()` and pins only the slug `demo`), so the leg had been quietly red since Phase 3 and the README repeated the dead URL. `verify-shell.ts` now resolves the id by slug through the service role; the README sends the operator to `/campaigns`. Resolved by lookup rather than by scraping the list page because the only `/campaigns/<id>` link lives inside a Base UI dropdown that portals in on open — it is not in the server HTML.
17. **Retain-on-a-never-published campaign publishes it.** `snapshot_live_pages` was widened in Phase 11 (finding 14) to the manifest-eligible set, so checking "also keep the published landing page(s)" on a campaign whose pages are merely `pending` creates `retained_pages` rows for pages that were never live — permanently publishing them on the next deploy.
18. **Domain reservation in `leads`.** Global dedupe on `leads.domain` means every domain ever imported is permanently unusable for a fresh-recording lead; a future fixture reusing one silently skips the recorder (`import_commit_fn.sql:128-140`, `worker/steps/record.ts:85`).
